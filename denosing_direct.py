...
def main():
    ...
    for epoch in range(0,opt.epochs):
        ...
        for i, (data, label)in enumerate(loader_train, 0):
            ...
            loss_mse = criterion(out_train, img_train) / (imgn_train.size()[1]*2)
            out_train = torch.clamp(out_train,-2,2)
            ...
            save_imgs = torch.cat((save_imgs[[0,1,2]], out_train.cpu()[[0,1,2]]), dim=0)
            ...
            if step % 200 == 0:
                ...
                with torch.no_grad():
                    ...
                    for k,(data_val,label_val) in enumerate(loader_val):
                        ...
                        out_val = torch.clamp(model(data_val).cpu(),-2,2)
        ...
        # save model
        torch.save(model.state_dict(), os.path.join(opt.outf, 'net_17_direct_segy_25num.pth'))
